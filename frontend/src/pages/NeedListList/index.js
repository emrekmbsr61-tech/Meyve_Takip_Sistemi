...
export default function NeedListList({ currentUser }) {
  ...
  /*
    Yalnızca MAGAZA_PERSONELI düzenleyip silebilir; ADMIN salt okunur görür.

    Not: Bu ekran artık MAGAZA_MUDURU menüsünde yoktur; müdür planları
    "Alım İşlemleri" ekranından yönetir.
  */
  const canManage = currentUser.role === "MAGAZA_PERSONELI";

  /*
    Plana sonradan ürün ekleme penceresi. Personel planı oluşturduktan sonra
    unuttuğu bir ürünü buradan ekleyebilir; önceden bu mümkün değildi ve planı
    silip baştan oluşturmak gerekiyordu.
  */
  ...
  */
  ...
  const plans = useMemo(() => {
    ...
    const grouped = Object.values(visibleNeedLists.reduce((all, item) => {
      ...
      if (!all[groupKey]) all[groupKey] = { planId: item.planId, storeId: item.storeId, storeName: item.storeName, createdDate: item.createdDate, createdByName: item.createdByName, status: item.status, notes: item.planNotes, editable: item.editable, items: [] };
  ...
  // Seçilen ürünleri, YENİ plan açmadan aynı planId'ye ekler.
  const saveExtraItems = async (items) => {
    ...
    try {
      setExtraSaving(true);
      setExtraError("");
      await addExtraItemsToPlan(extraModalPlan.planId, { userId: currentUser.id, items });
      setExtraModalPlan(null);
      setMessage("Ürünler plana eklendi.");
      loadNeedLists();
    } catch (error) {
  ...
  return <ScrollView style={styles.container} contentContainerStyle={styles.content}>
    ...
    {visiblePlans.map((plan) => {
      ...
      {/*
        Alım yapıldıysa plan KİLİTLİDİR: düzenle/sil/ürün ekle gösterilmez.
        Sebebi denetim kuralıdır - müdür o miktara bakarak alım yaptı, sonradan
        değiştirilirse kayıp hesabı yanlış çıkar (bkz. backend
        NeedListService.requirePlanNotPurchased).
      */}
      {canManage && plan.editable === false ? <View style={styles.lockedBox}><Ionicons name="lock-closed-outline" size={16} color={colors.muted}/><Text style={styles.lockedText}>Alım yapıldı, bu plan artık değiştirilemez.</Text></View> : null}
      {canManage && plan.editable !== false ? (editingPlanId === plan.planId ? <View style={styles.buttonRow}><Pressable style={styles.saveSmall} onPress={() => saveEdit(plan)}><Text style={styles.smallText}>Kaydet</Text></Pressable><Pressable style={styles.cancelSmall} onPress={cancelEdit}><Text style={styles.smallText}>İptal</Text></Pressable></View> : <><View style={styles.buttonRow}><Pressable style={styles.editButton} onPress={() => startEdit(plan)}><Ionicons name="pencil-outline" size={19} color={colors.green}/><Text style={styles.editText}>Düzenle</Text></Pressable><Pressable style={styles.deleteButton} onPress={() => deletePlan(plan)}><Ionicons name="trash-outline" size={19} color={colors.red}/><Text style={styles.deleteText}>Sil</Text></Pressable></View><Pressable style={styles.addButton} onPress={() => { setExtraError(""); setExtraModalPlan(plan); }}><Ionicons name="add-circle-outline" size={19} color={colors.green}/><Text style={styles.addText}>Ürün Ekle</Text></Pressable></>) : null}
    </View>;
    })}
    ...
    <AddExtraProductModal
      ...
      onClose={() => setExtraModalPlan(null)}
    />
  </ScrollView>;
}
...
const styles = StyleSheet.create({
  ...
  lockedBox: { flexDirection: "row", alignItems: "center", gap: 7, backgroundColor: colors.background, borderRadius: 12, padding: 11, marginTop: 12 },
  lockedText: { flex: 1, color: colors.muted, fontSize: 12.5, fontWeight: "600" },
  addButton: { flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 7, borderWidth: 1, borderStyle: "dashed", borderColor: colors.green, borderRadius: 12, paddingVertical: 11, marginTop: 8 },
  addText: { color: colors.green, fontWeight: "800", fontSize: 14 },
});